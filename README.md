...
- **`src/lib/sim.smoke.ts`** — invariant assertions (totals reconcile, principal ≤ balance, etc.) across all 4 preset scenarios. Also tests storage round-trip and cap/cascade behavior.
- **`src/lib/expected-snapshots.ts`** — frozen reference outputs for each preset at every year. Auto-generated by `npm run capture-snapshots`. If the engine math intentionally changes, re-run that script and commit the updated snapshot file. If it changes unintentionally, the test fails loudly.
